...
def search_index(query, top_unique=3, fetch_limit=15):
    ...
    results = collection.query(query_embeddings=query_embedding, n_results=fetch_limit)
    ...
    if matches_found == 0:
        print(
            "No highly relevant matches found (results were below the quality threshold)."
        )
